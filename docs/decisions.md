...
## ADR-027: `_openingRef` — guard síncrono contra double-tap em ticket handlers

**Date:** 2026-07-22

### Problem

`StoreScreen` usava `ticketBusy` (state React) como única proteção contra aberturas duplicadas de ticket. Um double-tap de ~187ms disparava dois handlers antes do primeiro `setTicketBusy(true)` propagar um re-render, causando dois rolls simultâneos — consumindo dois tickets pelo preço de um clique.

Adicionalmente, se o callback de abertura lançasse uma exceção, `ticketBusy` ficava preso em `true` e o estudante não conseguia mais abrir tickets até recarregar a página.

### Decision

Adicionar `const _openingRef = useRef(false)` em `StoreScreen` como flag síncrona de exclusão mútua. Todos os handlers de abertura de ticket (`handleConfirmOpen`, `handleDirectOpen`, `handleOpenDawn`, `handleKeep`, `handleRefund`) seguem o padrão:
...
Criar `handleOpenDawn()` com o mesmo padrão para o Dawn Ticket. Os dois pontos no carousel que chamavam `onOpenDawnTicket?.()` diretamente passam a chamar `handleOpenDawn()`.

### Rationale

- `useRef` é síncrono — o valor muda no mesmo tick do evento, antes de qualquer re-render. `useState` não oferece essa garantia: dois eventos em sequência rápida leem o mesmo valor stale antes do primeiro `setState` propagar.
- `try/finally` garante que o guard reseta em qualquer cenário (sucesso, erro, rejeição de Promise) — elimina o risco de estado travado.
- `ticketBusy` (state) é mantido para a UI reagir visualmente (desabilitar botões, mostrar "…") — o ref e o state têm papéis complementares, não se substituem.

### Consequences

- Qualquer novo handler de abertura de ticket adicionado futuramente deve seguir o mesmo padrão `_openingRef` + `try/finally`.
- `ticketBusy` permanece como fonte de verdade para a UI; `_openingRef` é uma proteção de runtime que não precisa aparecer no JSX.

---

## ADR-028: `catalogVersion` — propagação explícita do cache de avatars customizados para `StoreScreen`

**Date:** 2026-07-22

### Problem

`forceCatalogRerender` era declarado como `const [, forceCatalogRerender] = useState(0)` — o contador era descartado (vírgula líder). Quando `refreshCustomAvatarCache()` resolvia e chamava `forceCatalogRerender(n => n + 1)`, o App re-renderizava, mas o React não tinha motivo explícito para re-renderizar `StoreScreen`: nenhuma de suas props tinha mudado. Como resultado, `getFullAvatarCatalog()` era chamado dentro de `StoreScreen` ainda com `_customAvatarCache` vazio, e a contagem de avatars no `sch-rarity-track` ficava errada até a próxima interação que forçasse um re-render independente.

### Decision

Capturar o contador: `const [catalogVersion, forceCatalogRerender] = useState(0)`.

Passar `catalogVersion={catalogVersion}` como prop para `<StoreScreen>`.

`StoreScreen` aceita `catalogVersion = 0` na assinatura (sem usar o valor internamente — a presença da prop como dependência do React é suficiente para forçar o re-render no momento certo).

### Rationale

- React re-renderiza um componente filho quando qualquer prop muda. Passar `catalogVersion` como prop cria um canal explícito: quando o cache de gatos customizados carrega, `forceCatalogRerender` incrementa o contador, a prop muda, e `StoreScreen` re-renderiza e chama `getFullAvatarCatalog()` com `_customAvatarCache` já populado.
- A alternativa (usar `useEffect` + `forceUpdate` dentro de `StoreScreen`) acoplaria o componente ao ciclo de vida do cache sem uma interface clara. A prop é mais declarativa e rastreável.
- O valor de `catalogVersion` em si não precisa ser consumido — é o fato de a prop ter mudado que importa.

### Consequences

- Qualquer outro componente que use `getFullAvatarCatalog()` e precise reagir ao carregamento do cache deve receber `catalogVersion` como prop pelo mesmo padrão.
- O nome `catalogVersion` é intencional: documenta que o número representa uma versão do catálogo, não um estado arbitrário de UI.
